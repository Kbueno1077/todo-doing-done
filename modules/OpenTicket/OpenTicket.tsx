...
const mockComments = [
  {
    id: "a6a56c54-b488-4e1b-b225-7ae448230269",
    content: "Hello There",
    updated_at: "2025-04-26T00:10:52.311Z",
    created_at: "2025-04-26T00:10:52.311Z",
    Users: {
      id: "DEMO-ID-USER",
      name: "Guest",
      img: "",
    ...
    author_id: "DEMO-ID-USER",
    ticket_id: "74338731-9652-43e3-a490-bfc072ed702d",
  },

  {
    id: "b7c67d65-c599-5f2c-c336-8bf559341370",
    content: "Great Day",
    updated_at: "2025-04-26T00:10:55.311Z",
    created_at: "2025-04-26T00:10:55.311Z",
    Users: {
      id: "DEMO-ID-USER-2",
      name: "Guest 2",
      img: "",
...
function OpenTicket({ ticket, index }: AddTicketProps) {
  ...
  const handleFormChange = (name: keyof CreateBoardFormData, value: string) => {
  ...
  const toggleUser = (user: User) => {
    setSelectedUsers((prevSelected) => {
      const isCurrentlySelected = prevSelected.some((u) => u.id === user.id);
  ...
  async function handleSubmit() {
    ...
    if (!result.success) {
      const newErrors: Partial<Record<keyof CreateBoardFormData, string>> = {};
      result.error.issues.forEach((issue) => {
        if (issue.path[0]) {
          newErrors[issue.path[0] as keyof CreateBoardFormData] = issue.message;
        }
      });
      setErrors(newErrors);
      return;
    ...
    const isAssignedUpdateNeeded =
      ...
      !(ticket.AssignedToTickets || []).every((assigned) =>
        (selectedUsers || []).some((sel) => assigned.Users.id === sel.id)
      ) ||
                ...
                {!isDeleteOpen && (
                  <IpadCursorBlockWrapper>
                    ...
                    >
                      <IconTrash
                        size={20}
                        className={`${isDeleteOpen && "text-warning"}`}
                      />
                    </button>
                  </IpadCursorBlockWrapper>
                )}
              ...
              {!isDeleteOpen ? (
                <>
                  <div className="flex flex-col gap-4 py-4">
                    <div className="flex flex-col items-start sm:flex-row gap-2 sm:gap-4">
                      <div className="flex items-start gap-2">
                        <IconH1 size={30} />
                        <h3 className="sm:hidden font-bold text-xl">Title</h3>
                      </div>
                  ...
                  <div className="flex gap-2">
                              ...
                              <IpadCursorBlockWrapper>
                                <label className="flex items-center space-x-2">
                                  <button
                                    ...
                                    onClick={() => setSelectedStatus(column[0])}
                                  >
                                    <input
                                      type="radio"
                                      name="column-radio"
                                      className="radio radio-primary"
                                      checked={selectedStatus === column[0]}
                                      onChange={() =>
                                        setSelectedStatus(column[0])
                                      }
                                      value={column[0]}
                                    />
                    ...
                    <div id="AssigneedDropDown" className="dropdown">
                      <div className="flex gap-2 items-center">
                        <button
                          className="btn m-1"
                          disabled={isLoading}
                          onClick={openAssigneedDropDown}
                        >
                          Assign To ({selectedUsers.length})
                        </button>

                        {selectedUsers.length > 0 && (
                          <GroupedAvatars assignedTo={selectedUsers} />
                        )}
                      </div>
                    ...
                    <div className="my-4">
                      <div className="collapse bg-base-200 sm:px-3">
                        <input
                          ...
                          style={{
                            zIndex:
                              isDropDownOpen || isStatusDropDownOpen ? 0 : 10,
                          }}
                        />
                          ...
                          {comments?.map((comment, index) => (
                            <div
                              ...
                              className={`flex items-center chat chat-start ${
                                loggedUser?.id === comment.Users.id ||
                                comment.Users.id === DEMO_USER_ID
                  ...
                  <div className="flex flex-col sm:flex-row  gap-2 sm:gap-4">
                    <IconMessage className="hidden sm:block" size={30} />
