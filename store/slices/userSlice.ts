...
import { mockUsers } from "@/mock/demoUsers";
...
export const createUserSlice = (set: Function, get: Function) => ({
  ...
  loadUsersFromBoard: async (boardId: string) => {
    try {
      ...
      const users = mockUsers.map((item) => item.Users).filter(Boolean);
  ...
  sendInvite: async (email: string): Promise<any | Error> => {
    try {
      if (IS_DEMO_ENV === process.env.NEXT_PUBLIC_IS_DEMO) {
        showToast("Inviting users is not available in the demo", "error");
        return;
      }
      ...
      if (getAllUsersData.user || invitationSent) {
        ...
        if (boardInsertData.error) {
          if (boardInsertData.error.code === "23505") {
            throw new Error("This user is already registered to this board");
          } else {
            throw new Error();
          }
        }
